...
# Always use ~/.terminator.env for API key storage
ENV_PATH = os.path.expanduser("~/.terminator.env")
...
def ensure_api_key():
    ...
    if not api_key or debug_mode:
        ...
        if new_key:
            ...
            print(f"API key saved to {ENV_PATH}. Please restart the application to enable full functionality.")
            exit(0)
    return debug_mode
